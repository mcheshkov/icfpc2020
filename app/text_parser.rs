        ...
        stream
            ...
            .map(|(token, rest)| {
                match token {
                    ...
                    Token::Apply() => {
                        let (first, rest) = Token::parse_next(rest).expect("Could not parse first argument for ap");
                        let (second, rest) = Token::parse_next(rest).expect("Could not parse second argument for ap");
        ...
        match self {
            Self::Call(val, mut args) if *val == MathAction::Value("cons") && args.len() == 2 => {
                ...
                match second_list_possibly {
                    ...
                    MathAction::Value("nil") => {
                        MathAction::List(vec!(first))
                    },
                    _ => MathAction::Call(Box::new(MathAction::Value("cons")), vec!(first, second_list_possibly)),
                }
